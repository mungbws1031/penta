...
// ===== MBTI 16유형 상세 서술 =====
export const MBTI_DEEP = {
  ENTJ: {
    catch: '목표가 정해지면 세상이 장애물 코스로 보이는 타고난 지휘관',
    vibe: '회의 시간에 안건이 산으로 가면 참다못해 직접 정리해버린다. 계획 없이 떠나는 여행, 리더 없이 굴러가는 팀 프로젝트를 세상에서 가장 답답해하고, 대화 중에도 머릿속으로 이미 세 수 앞의 전략을 짜고 있다. 비효율이 눈에 보이면 몸이 근질거려 참견하지 않고는 못 배긴다.',
    strength: '목표를 세우면 사람과 자원을 조직해 끝까지 밀어붙이는 추진력이 타의 추종을 불허한다. 어려운 결정을 남에게 미루지 않고 스스로 짊어지는 책임감이 주변에 큰 안정감을 준다.',
    shadow: '효율이 떨어지는 사람이나 절차를 보면 참을성이 바닥나 날카로운 말이 먼저 튀어나온다. 결과를 우선하다 보니, 정작 옆에서 지쳐가는 사람의 마음을 놓치기 쉽다.',
    match: '같은 직진성을 가진 ENFP·INTP와는 아이디어를 실행으로 옮기는 환상의 파트너가 되고, INFJ와는 비전을 공유하며 깊이 신뢰한다. 다만 ISFP처럼 감정과 속도를 중시하는 유형과는 "왜 이렇게 오래 걸리냐"는 조바심 때문에 자주 부딪힌다.',
  },
  ENTP: {
    catch: '반박거리를 찾는 순간 눈이 반짝이는 아이디어 뱀장어',
    vibe: '누가 "그건 원래 이렇게 하는 거야"라고 하면 자동으로 "왜?"가 튀어나온다. 하나의 대화 속에서 화제를 몇 번이고 비틀고, 스스로도 놀랄 즉흥 논리를 즐긴다. 새 프로젝트를 시작할 때의 흥분은 세상 누구보다 강하지만, 서류 정리 같은 반복적인 일은 좀처럼 견디지 못한다.',
    strength: '고정관념을 비틀어 아무도 생각 못 한 각도의 아이디어를 던지는 순발력이 탁월하다. 논쟁 자체를 즐기기 때문에 압박 속에서도 유연하게 말을 이어가는 재치가 있다.',
    shadow: '시작만 화려하고 마무리가 약해 벌여놓은 일이 쌓이기 쉽다. 반박을 위한 반박을 하다 상대를 지치게 하고, 정작 자신이 뱉은 말에 책임지는 걸 잊을 때가 있다.',
    match: 'ENTJ·INTJ와는 아이디어와 실행력이 만나 시너지가 크고, INFP와는 서로의 엉뚱함을 알아봐 주며 즐겁다. 반면 ISTJ·ISFJ처럼 원칙과 안정을 중시하는 유형에게는 즉흥성이 무례하게 비칠 수 있어 마찰이 생긴다.',
  },
  ENFJ: {
    catch: '옆 사람 마음이 먼저 눈에 들어오는 타고난 무대 감독',
    vibe: '친구 표정이 살짝만 흐려져도 바로 알아채고 "무슨 일 있어?"를 묻는다. 모임에서는 자연스럽게 다들 잘 어울리도록 자리를 배치하고 대화를 이끈다. 자신의 컨디션보다 남의 필요를 먼저 챙기다 정작 자기 얘기는 뒷전이 되곤 한다.',
    strength: '사람의 잠재력을 알아보고 진심으로 응원하는 힘이 있어, 곁에 있으면 없던 자신감도 생긴다. 조직 전체의 온도를 감지해 갈등을 미리 조율하는 사교적 리더십이 탁월하다.',
    shadow: '모두를 만족시키려다 자신의 에너지를 소진하고, 거절을 못 해 부탁을 떠안는다. 인정받지 못한다고 느끼면 의외로 크게 상처받고 오래 곱씹는다.',
    match: 'INFP·ISFP처럼 섬세한 감성을 가진 유형과는 서로를 깊이 이해하며, ENFP와는 함께 있으면 에너지가 배가 된다. 반면 INTP처럼 감정 표현에 인색한 유형과는 애정을 확인받지 못해 서운함이 쌓이기 쉽다.',
  },
  ENFP: {
    catch: '머릿속 불꽃놀이가 멈추지 않는 자유로운 영감러',
    vibe: '새로운 사람, 새로운 가능성을 보면 눈이 커지고 심장이 뛴다. 하고 싶은 일이 하루에도 여러 개씩 생기고, 그중 몇 개는 바로 실행에 옮긴다. 틀에 박힌 일상, 반복되는 루틴을 세상에서 가장 못 견딘다.',
    strength: '누구와도 금세 친해지는 따뜻한 친화력과 상상력이 뿜어져 나오는 아이디어 뱅크다. 사람들의 숨은 가능성을 알아채고 진심으로 응원해 주는 힘이 있다.',
    shadow: '일을 벌이는 속도가 마무리하는 속도보다 늘 빠르다. 감정 기복이 크고, 관심이 쉽게 옮겨가 시작한 일을 흐지부지 두는 경우가 많다.',
    match: 'INTJ·INFJ처럼 깊이 있는 유형과는 서로 부족한 부분을 채우며 오래가고, ENFJ와는 함께 있으면 에너지가 폭발한다. 반면 ISTJ처럼 원칙과 계획을 중시하는 유형과는 즉흥성 때문에 자주 답답함을 주고받는다.',
  },
  ESTJ: {
    catch: '규칙과 순서가 세상을 돌아가게 한다고 믿는 현장 관리자',
    vibe: '약속 시간에 1분만 늦어도 신경이 쓰이고, 정해진 절차를 건너뛰는 사람을 보면 한마디 하고 싶어진다. 집안이든 회사든 시스템이 없으면 만들어서라도 정리하고야 만다. 결과로 증명하는 걸 최고의 미덕으로 여긴다.',
    strength: '조직을 체계적으로 굴리는 실행력과 책임감이 발군이다. 위기 상황에서도 흔들리지 않고 명확한 지시로 상황을 정리하는 안정감을 준다.',
    shadow: '규칙에서 벗어나는 방식을 쉽게 인정하지 못해 융통성이 부족해 보일 수 있다. 효율을 앞세우다 사람 마음을 다치게 하고도 잘 눈치채지 못한다.',
    match: 'ISTJ·ESFJ처럼 성실하고 조직적인 유형과는 신뢰를 바탕으로 오래가고, ENTJ와는 서로의 추진력을 존중하며 시너지를 낸다. 반면 INFP처럼 감정과 개인의 속도를 중시하는 유형과는 원칙 대 융통성 문제로 부딪히기 쉽다.',
  },
  ESTP: {
    catch: '생각보다 몸이 반 박자 빠른 지금 이 순간의 승부사',
    vibe: '위기 상황일수록 눈이 빛나고 몸이 먼저 반응한다. 계획을 세우기보다 일단 부딪혀 보고 그 자리에서 수습하는 편이 편하다. 새로운 자극, 짜릿한 스릴을 못 참아 늘 뭔가 벌이고 있다.',
    strength: '실전 감각과 순발력이 탁월해 돌발 상황에서 가장 믿음직한 사람이 된다. 눈치가 빠르고 현실적이라 사람들 사이에서 분위기를 실용적으로 조율한다.',
    shadow: '장기적인 계획이나 반복적인 준비 과정을 지루해해 중요한 걸 놓치기도 한다. 순간의 즐거움을 좇다 결과를 깊이 생각하지 않고 저지르는 경향이 있다.',
    match: 'ISFJ·ISTJ처럼 안정적인 유형과는 서로의 빈틈을 채워주고, ESFP와는 함께 있으면 세상 즐거운 콤비가 된다. 반면 INFJ처럼 깊고 신중한 유형과는 속도 차이 때문에 서로 답답해하기 쉽다.',
  },
  ESFJ: {
    catch: '모두가 편안한지 항상 레이더를 켜두는 분위기 지킴이',
    vibe: '모임을 주최하면 누가 뭘 못 먹는지까지 미리 챙긴다. 사람들 사이의 미묘한 온도 변화를 가장 먼저 알아채고, 다투는 사람이 있으면 나서서 중재한다. 예의와 매너를 중요하게 여겨 무례한 사람을 유독 못 참는다.',
    strength: '따뜻한 배려와 성실함으로 주변을 챙기는 힘이 탁월해 어디서나 신뢰를 얻는다. 전통과 관계를 소중히 여겨 공동체를 단단하게 묶어내는 역할을 자연스럽게 맡는다.',
    shadow: '남의 평가에 예민해 인정받지 못하면 크게 흔들린다. 조화를 위해 자기 의견을 억누르다 정작 자신이 무엇을 원하는지 잊어버리기도 한다.',
    match: 'ISFJ·ESTJ처럼 성실하고 관계를 중시하는 유형과는 잘 맞고, ISFP와는 서로를 다정하게 챙기며 안정적이다. 반면 INTP처럼 독립적이고 감정 표현이 적은 유형과는 서운함이 쌓이기 쉽다.',
  },
  ESFP: {
    catch: '지금 이 순간을 축제로 만드는 무대 위 즉흥 배우',
    vibe: '음악이 나오면 몸이 먼저 반응하고, 사람들 앞에서 주목받는 걸 즐긴다. 계획보다 오늘의 기분과 재미를 우선하며, 우울한 분위기를 견디지 못해 어떻게든 웃음을 만들어낸다. 예쁘고 화려한 것에 본능적으로 끌린다.',
    strength: '타고난 유쾌함과 현장감으로 분위기를 순식간에 살리는 힘이 있다. 사람들의 감정을 빠르게 캐치하고 함께 즐기며 주변까지 밝게 만드는 전염성 있는 에너지를 가졌다.',
    shadow: '미래를 위한 준비나 진지한 계획을 미루다 중요한 순간에 허둥댄다. 비판을 받으면 예상보다 크게 상처받고, 그 순간을 피하려 도망치는 경향이 있다.',
    match: 'ESTP·ENFP와는 텐션이 잘 맞아 함께 있으면 늘 즐겁고, ISFJ와는 서로의 빈 곳을 채워준다. 반면 INTJ처럼 계획과 논리를 앞세우는 유형과는 즉흥성 때문에 자주 부딪힌다.',
  },
  INTJ: {
    catch: '머릿속에 이미 완성된 설계도가 있는 고독한 전략가',
    vibe: '회식보다 혼자 책 읽는 밤을 훨씬 선호하고, 목표를 정하면 몇 년 뒤까지의 계획을 조용히 그려둔다. 비효율적인 잡담보다 핵심만 짚는 대화를 좋아해, 가끔 차갑다는 오해를 받는다. 한번 확신이 서면 주변의 반대에도 흔들리지 않는다.',
    strength: '복잡한 문제를 구조화해 장기적인 전략을 세우는 통찰력이 탁월하다. 남의 인정 없이도 스스로 방향을 밀고 나가는 독립성과 뚝심이 있다.',
    shadow: '자신의 논리에 확신이 강해 다른 관점을 쉽게 수용하지 못한다. 감정을 표현하는 데 서툴러 가까운 사람에게조차 속마음을 다 보여주지 않는다.',
    match: 'ENTP·ENFP처럼 개방적이고 창의적인 유형과는 서로 자극을 주고받으며 잘 맞고, INFJ와는 깊은 대화로 신뢰를 쌓는다. 반면 ESFP처럼 즉흥적이고 감정 표현이 큰 유형과는 속도와 방식이 달라 서로 피곤해지기 쉽다.',
  },
  INTP: {
    catch: '질문 하나를 파고들다 밤을 새우는 논리의 탐험가',
    vibe: '궁금한 게 생기면 답을 찾을 때까지 검색과 사색을 멈추지 못한다. 겉모습이나 규칙보다 그 이면의 원리에 관심이 많아, "왜 그렇게 되는지" 설명 못 하는 걸 참지 못한다. 정리정돈이나 일상적인 잡무는 뒷전이 되기 쉽다.',
    strength: '복잡한 개념을 분해해 본질을 꿰뚫는 분석력이 탁월하다. 고정관념에 얽매이지 않는 독창적인 사고로 아무도 못 본 연결고리를 찾아낸다.',
    shadow: '생각의 세계에 빠져 현실적인 마감이나 감정 교류를 소홀히 하기 쉽다. 논리적으로 맞지 않으면 상대의 감정을 배려하지 못하고 직설적으로 말해 상처를 줄 수 있다.',
    match: 'ENTJ·ENTP처럼 직진성 있는 유형과는 아이디어를 실행으로 옮기는 좋은 파트너가 되고, INFJ와는 서로의 깊이를 알아본다. 반면 ESFJ처럼 감정과 관계를 우선하는 유형과는 무심함 때문에 오해가 쌓이기 쉽다.',
  },
  INFJ: {
    catch: '말하지 않아도 마음을 먼저 읽어버리는 조용한 예언자',
    vibe: '상대가 말하지 않은 감정까지 표정에서 읽어내고, 혼자 있는 시간에 그 의미를 오래 곱씹는다. 겉으로는 조용하지만 속에는 세상을 더 낫게 만들고 싶다는 뜨거운 이상이 있다. 소수의 깊은 관계를 선호하고, 얕은 인맥에는 쉽게 지친다.',
    strength: '사람의 마음을 통찰하는 직관과 진심 어린 공감력이 탁월해, 곁에 있으면 이해받는 느낌을 준다. 이상을 향해 조용히 그러나 끝까지 밀고 나가는 내적 신념이 강하다.',
    shadow: '완벽한 이상과 현실 사이의 간극에서 쉽게 지치고 소진된다. 갈등을 극도로 피하려다 속마음을 감추고 혼자 끙끙 앓는 경우가 많다.',
    match: 'INFP·ENFP처럼 이상을 공유하는 유형과는 깊게 통하고, INTJ와는 비전을 나누며 신뢰를 쌓는다. 반면 ESTP처럼 즉흥적이고 직설적인 유형과는 서로의 속도와 표현 방식이 달라 부담을 느끼기 쉽다.',
  },
  INFP: {
    catch: '마음속에 자기만의 우주를 품고 사는 이상주의자',
    vibe: '영화나 소설 속 인물에 감정을 이입해 며칠을 앓기도 한다. 겉으로 표현은 안 해도 마음속에는 옳고 그름에 대한 확고한 기준이 있다. 사람 많은 자리보다 혼자 생각을 정리하는 시간이 훨씬 편하다.',
    strength: '깊은 공감력과 풍부한 상상력으로 사람과 이야기의 이면을 이해하는 힘이 있다. 자신만의 가치관을 지키며 조용히 진심을 실천하는 성실함이 있다.',
    shadow: '현실적인 마감이나 갈등 상황을 마주하면 회피하고 싶어진다. 자기 기준에 못 미치면 스스로를 심하게 자책하고, 비판에 유독 예민하게 반응한다.',
    match: 'INFJ·ENFP처럼 깊은 감성을 공유하는 유형과는 서로를 있는 그대로 이해하고, ENTP와는 엉뚱한 상상력이 잘 통한다. 반면 ESTJ처럼 원칙과 효율을 앞세우는 유형과는 가치관 차이로 부딪히기 쉽다.',
  },
  ISTJ: {
    catch: '한번 정한 원칙은 끝까지 지키는 조용한 원칙주의자',
    vibe: '계획을 세우면 반드시 지키고, 규칙과 절차를 존중한다. 화려한 말보다 꾸준한 행동으로 신뢰를 쌓는 편이며, 갑작스러운 변화보다 예측 가능한 흐름을 선호한다. 맡은 책임은 아무리 지루해도 끝까지 완수한다.',
    strength: '성실함과 꾸준함으로 누구보다 믿음직한 사람이 된다. 세부 사항을 놓치지 않는 꼼꼼함으로 일을 정확하게 처리하는 힘이 있다.',
    shadow: '변화나 예상치 못한 상황에 유연하게 대응하기 어려워한다. 감정 표현이 서툴러 무뚝뚝해 보이고, 새로운 방식을 받아들이는 데 시간이 오래 걸린다.',
    match: 'ESTJ·ISFJ처럼 성실하고 원칙적인 유형과는 안정적으로 잘 맞고, ESTP와는 서로의 빈틈을 채워준다. 반면 ENFP처럼 즉흥적이고 자유로운 유형과는 계획 대 즉흥 문제로 자주 답답해한다.',
  },
  ISTP: {
    catch: '말보다 손이 먼저 움직이는 조용한 장인',
    vibe: '고장 난 물건을 보면 일단 분해부터 하고 본다. 필요 없는 말은 거의 하지 않지만, 위기 상황에서는 누구보다 침착하게 대응한다. 정해진 틀보다 자유롭게 움직일 수 있는 상황을 선호한다.',
    strength: '손과 몸으로 문제를 해결하는 실전 감각이 탁월하다. 어떤 상황에서도 침착하게 핵심만 파악해 효율적으로 대응하는 힘이 있다.',
    shadow: '감정을 표현하거나 나누는 데 서툴러 가까운 사람도 답답해할 수 있다. 관심 없는 일에는 무심해져 약속이나 마감을 소홀히 하기도 한다.',
    match: 'ESTP·ISTJ처럼 실용적인 유형과는 잘 맞고, ESFP와는 서로의 즉흥성이 통한다. 반면 ENFJ처럼 감정과 관계에 몰입하는 유형과는 표현 방식의 차이로 서운함이 쌓이기 쉽다.',
  },
  ISFJ: {
    catch: '묵묵히 곁을 지키며 다 기억해두는 조용한 수호자',
    vibe: '친구의 생일, 좋아하는 음식, 사소한 취향까지 기억해 뒀다가 챙긴다. 나서지 않지만 뒤에서 필요한 걸 미리 준비해 두는 세심함이 있다. 갈등을 극도로 피하고, 남에게 부담 주는 걸 가장 싫어한다.',
    strength: '헌신적인 배려와 꾸준한 성실함으로 곁에 있는 사람을 든든하게 만든다. 세부적인 것까지 놓치지 않는 관찰력으로 필요한 순간에 정확히 도움을 준다.',
    shadow: '자기 필요보다 남을 먼저 챙기다 정작 자신을 돌보지 못한다. 서운함을 표현하지 못하고 속으로 삭이다 관계에 벽을 만들기도 한다.',
    match: 'ESFJ·ISTJ처럼 성실하고 배려심 있는 유형과는 편안하게 잘 맞고, ESFP와는 서로를 밝게 채워준다. 반면 ENTP처럼 즉흥적이고 직설적인 유형과는 배려받지 못한다는 서운함이 쌓이기 쉽다.',
  },
  ISFP: {
    catch: '자유로운 감성으로 세상을 그려내는 조용한 예술가',
    vibe: '말보다 색깔, 소리, 분위기로 자신을 표현하는 게 편하다. 자기만의 취향과 미적 감각이 뚜렷하고, 억지로 정해진 틀에 맞추는 걸 힘들어한다. 겉으로 조용해도 마음속에는 자유를 향한 갈망이 크다.',
    strength: '섬세한 감수성과 미적 감각으로 예술적인 결과물을 자연스럽게 만들어낸다. 겸손하고 온화해 함께 있으면 편안한 분위기를 만든다.',
    shadow: '갈등을 극도로 피하려다 정작 필요한 말을 못 하고 넘어간다. 즉흥적인 감정에 따라 움직이다 장기적인 계획을 놓치기 쉽다.',
    match: 'ISFJ·ESFJ처럼 다정한 유형과는 편안하게 잘 맞고, ENFJ와는 서로의 감성을 알아봐 준다. 반면 ESTJ처럼 원칙과 효율을 앞세우는 유형과는 자유로운 성향이 부딪히기 쉽다.',
  },
};

export function mbtiNarrative(mbti) {
  const key = String(mbti || '').trim().toUpperCase();
  if (!/^[EI][NS][TF][JP]$/.test(key)) return '';
  const m = MBTI_DEEP[key];
  if (!m) return '';
  let html = '';
  html += `<p class="z-catch">"${m.catch}"</p>`;
  html += `<p class="nv-head">✨ 이런 사람이에요</p><p>${m.vibe}</p>`;
  html += `<p class="nv-head nv-confirm">💪 타고난 매력</p><p>${m.strength}</p>`;
  html += `<p class="nv-head nv-caution">🌑 숨은 그림자</p><p>${m.shadow}</p>`;
  html += `<div class="z-match"><div class="z-match-row"><span class="z-match-k good">👍 궁합</span><span class="z-match-v">${m.match}</span></div></div>`;
  html += `<p class="nv-foot">성격심리학 통설(MBTI) 기반 재미용 풀이입니다. MBTI는 재검사 시 유형이 바뀌는 경우가 적지 않아 확정적 진단으로 보기 어려워요.</p>`;
  return html;
}

// ===== 혈액형 성격 통설 =====
export const BLOOD_DEEP = {
  A: {
    catch: '완벽하게 준비해야 마음이 놓이는 신중한 계획러',
    traits: '여행 가기 일주일 전부터 리스트를 만들고, 남들이 눈치 못 채는 작은 실수도 스스로는 오래 신경 쓴다. 예의와 규칙을 중요하게 여겨 약속 시간에 늦는 걸 유독 못 견딘다. 겉으로는 조용해도 속으로는 이미 여러 경우의 수를 그려둔 상태다.',
    strength: '꼼꼼함과 책임감으로 맡은 일을 빈틈없이 해내는 신뢰를 준다. 남을 배려하는 섬세함 덕분에 관계에서 오래가는 신용을 쌓는다.',
    shadow: '완벽주의가 지나쳐 작은 실수에도 스스로를 심하게 몰아붙인다. 남의 시선을 지나치게 의식해 정작 자기 마음은 뒷전이 되기도 한다.',
  },
  B: {
    catch: '마음이 끌리는 대로 움직이는 자유로운 마이웨이',
    traits: '계획표보다 그날의 기분이 우선이고, 관심사가 생기면 앞뒤 안 재고 뛰어든다. 남의 눈치보다 자기 페이스를 지키는 게 훨씬 중요하고, 좋고 싫음이 얼굴에 다 드러난다. 틀에 박힌 규칙이나 잔소리를 세상에서 가장 못 견딘다.',
    strength: '자유분방한 발상과 몰입력으로 자기 분야에서 독보적인 결과를 낸다. 눈치 안 보고 소신대로 밀고 나가는 뚝심이 강점이다.',
    shadow: '관심 없는 일은 쉽게 흥미를 잃어 마무리가 약할 때가 많다. 자기 위주로 생각하다 주변 사람 감정을 놓치는 순간이 있다.',
  },
  O: {
    catch: '무리 속에서 자연스럽게 중심이 되는 타고난 리더',
    traits: '새로운 모임에 가도 금세 분위기를 파악하고 사람들을 이끈다. 목표가 생기면 승부욕이 발동해 끝까지 밀어붙이고, 대범한 결정을 내리는 데 주저함이 적다. 사람 좋아하고 마당발이라 어디서나 인맥이 넓다.',
    strength: '사교성과 통솔력으로 사람을 모으고 이끄는 힘이 탁월하다. 위기 상황에서도 대범하게 결단해 상황을 정리하는 리더십이 있다.',
    shadow: '승부욕이 지나쳐 지는 것에 유독 민감하고 고집이 세다. 자신감이 앞서 남의 조언을 흘려듣다 실수를 반복하기도 한다.',
  },
  AB: {
    catch: '냉철함과 몽상이 한 몸에 공존하는 이중적 천재',
    traits: '논리적으로 분석하다가도 갑자기 엉뚱한 상상에 빠진다. 겉으로는 쿨하고 거리감 있어 보이지만 속에는 남들이 모르는 섬세한 세계가 있다. 소수의 사람과는 깊게, 나머지와는 적당한 거리를 두는 이중적인 관계 방식을 가졌다.',
    strength: '이성과 감성, 현실과 이상을 오가는 독특한 균형 감각으로 창의적인 해법을 낸다. 다각도로 상황을 보는 분석력 덕분에 남들이 못 본 것을 짚어낸다.',
    shadow: '속을 잘 안 보여줘 종잡을 수 없다는 오해를 사고, 스스로도 어느 쪽이 진짜 나인지 헷갈릴 때가 있다. 극단을 오가는 감정 기복 때문에 주변이 혼란스러워하기도 한다.',
  },
};

export function bloodNarrative(blood) {
  const key = String(blood || '').trim().toUpperCase();
  if (!['A', 'B', 'O', 'AB'].includes(key)) return '';
  const b = BLOOD_DEEP[key];
  if (!b) return '';
  let html = '';
  html += `<p class="z-catch">"${b.catch}"</p>`;
  html += `<p class="nv-head">✨ 특징</p><p>${b.traits}</p>`;
  html += `<p class="nv-head nv-confirm">💪 강점</p><p>${b.strength}</p>`;
  html += `<p class="nv-head nv-caution">🌑 그림자</p><p>${b.shadow}</p>`;
  html += `<p class="nv-foot">혈액형 성격설은 과학적 근거가 없는 일본·한국의 통속 문화입니다(대규모 연구에서 상관관계가 확인되지 않았어요 — 아래 "과학적 근거 노트" 참고). 재미로만 봐주세요.</p>`;
  return html;
}
